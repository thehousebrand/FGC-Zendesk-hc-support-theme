(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    (function buildTOC() {
      ...
      if (!headings.length) {
        if (tocContainer.parentElement) tocContainer.parentElement.style.display = 'none';
        return;
      }
    ...
    (function nav() {
      ...
      collapsible.forEach((element) => {
        const toggle = qs(".collapsible-nav-toggle, .collapsible-sidebar-toggle", element);
    ...
    (function searchInit() {
      ...
      const searchInputs = searchForms.map((form) => qs("input[type='search']", form)).filter(Boolean);
    ...
    (function formsInit() {
      ...
      const commentContainerFormControls = qs(".comment-form-controls, .comment-ccs");
      ...
      const showRequestCommentContainerTrigger = qs(".request-container .comment-container .comment-show-container");
      const requestCommentFields = qsa(".request-container .comment-container .comment-fields");
      const requestCommentSubmit = qs(".request-container .comment-container .request-submit-comment");

      if (showRequestCommentContainerTrigger) {
        showRequestCommentContainerTrigger.addEventListener("click", () => {
          showRequestCommentContainerTrigger.style.display = "none";
          requestCommentFields.forEach((el) => { el.style.display = "block"; });
      ...
      const requestMarkAsSolvedButton = qs(".request-container .mark-as-solved:not([data-disabled])");
      const requestMarkAsSolvedCheckbox = qs(".request-container .comment-container input[type=checkbox]");
      const requestCommentSubmitButton = qs(".request-container .comment-container input[type=submit]");
      ...
      const requestCommentTextarea = qs(".request-container .comment-container textarea");
      const usesWysiwyg =
      ...
      if (requestCommentTextarea) {
        requestCommentTextarea.addEventListener("input", () => {
          if (isEmpty(requestCommentTextarea.value)) {
            ...
            if (requestMarkAsSolvedButton) {
              requestMarkAsSolvedButton.innerText =
                requestMarkAsSolvedButton.getAttribute("data-solve-and-submit-translation");
            }
          }
        });
      }

      const selects = qsa("#request-status-select, #request-organization-select");
      selects.forEach((element) => {
        element.addEventListener("change", (event) => {
          event.stopPropagation();
          const activeElementId = document.activeElement && document.activeElement.getAttribute("id");
